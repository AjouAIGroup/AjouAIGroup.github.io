...
이 project는 기존 content와 디자인을 유지하면서, route 단위 static HTML을 생성하는 **SSR + prerender pipeline**이 동작하는 상태입니다.
...
- build script:
    ...
    - `prerender`
    - `build:static`
- `entry-server`는 prerender 시 router `basename`을 적용하므로, subpath deploy(예: GitHub Pages repository path)에서도 생성 링크가 올바르게 유지됩니다.
...
2. **Client 전용 로직이 React page/components 내부에 포함되어 있습니다**
    - 인터랙션 모듈이 component 경계로 분리되어 있어도, 하나의 루트 앱 안에 함께 마운트됩니다.

3. **Router가 client-first(`BrowserRouter`) 구조를 유지합니다**
    - prerender된 HTML + client navigation 조합에서는 잘 동작합니다.
...
Vite + React를 유지하면서 현재 prerender pipeline을 확장합니다:
...
셸/page를 island를 지원하는 SSG 프레임워크(예: Astro)로 마이그레이션합니다:

1. 기존 JSON/content와 CSS 토큰을 유지합니다.
2. 인터랙션 파트는 React component를 island로 재사용합니다.
3. 기본은 정적 HTML을 렌더링하고, modal/nav/motion 위젯만 hydrate합니다.

이 방식이 현재 content와 디자인을 유지하면서 진짜 선택적 hydration으로 가는 가장 깔끔한 경로입니다.
